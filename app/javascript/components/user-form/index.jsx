...
const UserForm = ({
  ...
  useEffect(() => {
    if (id) {
      Promise.all([API.get('/api/groups?&expand=resources'), API.get(`/api/users/${id}`)])
        .then(([{ resources }, userData]) => {
          ...
          userData.groups = [userData.current_group_id];
          availableGroups.forEach((availableGroup) => {
            userData.groups.forEach((group) => {
              if (group === availableGroup.value) {
                selectedGroups.push(availableGroup.label);
              }
            });
          });
